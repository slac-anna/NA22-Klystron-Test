...
/////////////////////////////dimensions
    ...
    //simulation domain:
    G4double xWorld = 40.*mm;
    G4double yWorld = 40.*mm;
...
//Cu last cavity
//
    G4VSolid* solidUnion1 = new G4UnionSolid("CavityNLeftNC", solidCavity, solidLeftNC);
    G4VSolid* solidLastCavity = new G4UnionSolid("LastCavity", solidUnion1, solidRightNC);
    G4LogicalVolume *logicLastCavity = new G4LogicalVolume(solidLastCavity, CuMat, "logicalLastCavity");
    G4VPhysicalVolume *physLastCavity = new G4PVPlacement(0, G4ThreeVector(0., 0., offset), logicLastCavity, "physLastCavity", logicWorld, false, 0, true);
...
//*********************************************************************************************************
    ...
    G4LogicalVolume *logicVacuum = new G4LogicalVolume(solidVacuum, VaMat, "logicVacuum");
...
//
    ...
    for(G4int j=0; j<2; j++)
     {
       G4VPhysicalVolume *phyPoleLeft = new G4PVPlacement(0, G4ThreeVector(0., 0., (offset+j*17.506*mm)), logicPoleLeft, "physPoleLeft", logicWorld, false, j, true);
     }
...
//***********************************************************************************************************
    ...
    G4VSolid* solidDetector = new G4Tubs("solidDetector", out_body_r, (out_body_r+10), 3.753, 0., 360.*deg);
    ...
    //detector above the collector body for radiation monitor
    //G4VSolid* solidDetector = new G4Tubs("solidDetector", collector_out_r, (collector_out_r+20), (133.985-42.545)/2*mm, 0., 360.*deg);
    //G4LogicalVolume *logicDetector = new G4LogicalVolume(solidDetector, worldMat, "logicDetector");
    //G4VPhysicalVolume *physDetector = new G4PVPlacement(0, G4ThreeVector(0., 0., offset+(133.985+42.545)/2), logicDetector, "physDetector", logicWorld, false, 0, true);

//set a part as scoring volume

    //fScoringVolume = logicLastCavity;
